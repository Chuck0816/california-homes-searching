...
function cardTemplate(item) {
  ...
  const address = getFirst(item, ["Address", "L_Address", "address"], "Unknown address");
  ...
  const cover = photos[0] || "https://via.placeholder.com/600x400?text=No+Photo";

  return `
    <article class="home-card" data-id="${id}">
      <img class="home-img" src="${cover}" alt="home photo" loading="lazy" />
      <div class="home-body">
        <div class="home-price">${formatMoney(price)}</div>
        <div class="home-addr">${address}</div>
        <div class="home-meta">
          ${city}${zip ? ", " + zip : ""}
        </div>
        <div class="home-meta">
...
export function renderHouses(rows) {
  ...
  if (statsEl) {
    statsEl.textContent = `${rows.length} results (showing top ${Math.min(
      rows.length,
      100
    )})`;
  }
  ...
  if (rows.length === 0) {
    resultList.innerHTML = `<div class="muted">No results</div>`;
    return;
  }
  ...
  // 绑定点击打开详情
  resultList.querySelectorAll(".home-card").forEach((card) => {
...
export function showLoading() {
  if (resultList) {
    resultList.innerHTML = `<div class="muted">Loading...</div>`;
  }
...
function detailsTemplate(d) {
  ...
  return `
    ...
    <div class="details-price">${formatMoney(price)}</div>
    <div class="details-kv">
      <div><b>${beds}</b> beds</div>
      <div><b>${baths}</b> baths</div>
    ...
    ${
      remarks
        ? `<h3>Remarks</h3><p class="details-remarks">${remarks}</p>`
        : ""
    }
    ...
    <div class="details-contact">
      ...
      ${
        agentEmail
          ? `<div>Agent Email: <a href="mailto:${agentEmail}">${agentEmail}</a></div>`
          : ""
      }
      ${
        officeEmail
          ? `<div>Office Email: <a href="mailto:${officeEmail}">${officeEmail}</a></div>`
          : ""
      }
      ${
        mapsLink
          ? `<div><a target="_blank" rel="noopener" href="${mapsLink}">Open in Google Maps</a></div>`
          : ""
      }
    </div>
  `;
